...
class CriarEnderecoRestauranteComandoTest {
    ...
    @InjectMocks
    private CriarEnderecoRestauranteComando criarEnderecoRestauranteComando;

    private Usuario usuario;
    private Restaurante restaurante;
    private CriarEnderecoRestauranteComandoDto dto;
    ...
    void setUp() {
        ...
        dto = new CriarEnderecoRestauranteComandoDto();
        ...
        enderecoSalvo.setRestaurante(restaurante);
        enderecoSalvo.setDataCriacao(dataAtual);
    }

    @Test
    void deveCriarEnderecoRestauranteComSucesso() {
        // Arrange
        when(validarUsuarioExistente.execute(1L)).thenReturn(usuario);
        when(validarRestauranteExistente.execute(1L)).thenReturn(restaurante);
        ...
        // Act
        Endereco resultado = criarEnderecoRestauranteComando.execute(1L, dto);
        ...
        verify(validarRestauranteExistente).execute(1L);
    ...
    void deveValidarUsuarioExistenteAntesDeCriar() {
        ...
        // Act
        criarEnderecoRestauranteComando.execute(1L, dto);
    ...
    @Test
    void deveValidarRestauranteExistenteAntesDeCriar() {
        // Arrange
        when(validarUsuarioExistente.execute(1L)).thenReturn(usuario);
        when(validarRestauranteExistente.execute(1L)).thenReturn(restaurante);
        when(sharedService.getCurrentDateTime()).thenReturn(dataAtual);
